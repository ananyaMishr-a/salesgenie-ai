...
export default function LeadDashboard({ leads, onGenerateOutreach }) {
  ...
  const getScoreBadge = (score) => {
    ...
    if (score >= 75) return <span className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10">🔥 Hot ({score})</span>
    if (score >= 50) return <span className="inline-flex items-center gap-1 rounded-full bg-yellow-50 px-2 py-1 text-xs font-medium text-yellow-800 ring-1 ring-inset ring-yellow-600/20">☀️ Warm ({score})</span>
